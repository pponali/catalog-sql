...
public interface SellerProductRepository extends JpaRepository<SellerProduct, UUID> {
    ...
    List<SellerProduct> findBySellerIdOrderByCreatedDateDesc(UUID sellerId);
    
    // Find all products by seller for a specific merchant
    List<SellerProduct> findBySellerIdAndMerchantIdOrderByCreatedDateDesc(UUID sellerId, UUID merchantId);
