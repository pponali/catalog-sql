...
JAR_PATH="rules-service/target/${SERVICE_NAME}*.jar"
...
if [ ! -f $(ls ${JAR_PATH} 2>/dev/null | head -n 1) ]; then
  ...
  mvn clean package -DskipTests -f rules-service/pom.xml
fi
...
for i in {1..30}; do
  if grep -q "Started RulesServiceApplication" "${LOG_FILE}" 2>/dev/null; then
    ...
    if [ -n "${PORT}" ]; then
      ...
      echo "${SERVICE_NAME} running on port 8087"
    fi
