...
public class CatalogMigrationController {
    ...
    public ResponseEntity<MigrationResultDTO> bulkMigrate(
            ...
            @Valid @RequestBody MigrationRequestDTO request) {
        return ResponseEntity.ok(migrationService.bulkMigrate(request));
    }

    @GetMapping("/validate")
    @Operation(summary = "Validate migration possibility",
            description = "Check if migration between specified catalogs is possible")
    public ResponseEntity<Boolean> validateMigration(
            @Parameter(description = "Source catalog ID") @RequestParam UUID sourceCatalogId,
            @Parameter(description = "Target catalog ID") @RequestParam UUID targetCatalogId) {
        return ResponseEntity.ok(migrationService.validateMigration(sourceCatalogId, targetCatalogId));
    }

    @GetMapping("/history/{catalogId}")
    @Operation(summary = "Get migration history",
            description = "Retrieve migration history for a specific catalog")
    public ResponseEntity<List<MigrationResultDTO>> getMigrationHistory(
            @Parameter(description = "Catalog ID") @PathVariable UUID catalogId) {
        return ResponseEntity.ok(migrationService.getMigrationHistory(catalogId));
    }
}
