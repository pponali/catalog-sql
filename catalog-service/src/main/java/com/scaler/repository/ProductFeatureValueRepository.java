...
public interface ProductFeatureValueRepository extends JpaRepository<ProductFeatureValue, UUID> {
    ...
    @Query(value = "SELECT * FROM product_feature_value pfv WHERE pfv.feature_id = ?1", nativeQuery = true)
    List<ProductFeatureValue> findByFeatureId(UUID featureId);

    @Query(value = "SELECT * FROM product_feature_value pfv WHERE pfv.feature_id = ?1 AND pfv.id = ?2", nativeQuery = true)
    Optional<ProductFeatureValue> findByFeatureIdAndId(UUID featureId, UUID id);

    @Query(value = "SELECT * FROM product_feature_value pfv WHERE pfv.feature_id = ?1 AND pfv.attribute_values IS NOT NULL", nativeQuery = true)
    List<ProductFeatureValue> findByFeatureIdAndValueIsNotNull(UUID featureId);

    @Query(value = "SELECT * FROM product_feature_value pfv WHERE pfv.feature_id = ?1 AND pfv.type = ?2", nativeQuery = true)
    List<ProductFeatureValue> findByFeatureIdAndValueType(UUID featureId, String type);

    @Query(value = "SELECT * FROM product_feature_value pfv WHERE pfv.feature_id = ?1 AND pfv.attribute_values->>'value' = ?2", nativeQuery = true)
    ...
    @Query(value = "SELECT pfv.* FROM product_feature_value pfv " +
           "JOIN product_feature pf ON pfv.feature_id = pf.id " +
           "WHERE pfv.product_id = ?1 " +
           "AND pf.template_id = ?2", nativeQuery = true)
    List<ProductFeatureValue> findByProductSkuAndTemplateCode(String productSku, String templateCode);

    @Query(value = "SELECT pfv.* FROM product_feature_value pfv " +
           "WHERE pfv.feature_id = ?1 AND pfv.unit_of_measure = ?2", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM product_feature_value pfv WHERE pfv.type = ?1", nativeQuery = true)
    List<ProductFeatureValue> findByFeatureType(String type);

    @Query(value = "SELECT * FROM product_feature_value pfv WHERE pfv.validation_status = ?1", nativeQuery = true)
    List<ProductFeatureValue> findByValidationStatus(String status);

    @Query(value = "SELECT * FROM product_feature_value pfv WHERE pfv.validation_message LIKE CONCAT('%', ?1, '%')", nativeQuery = true)
    List<ProductFeatureValue> findByValidationMessageContaining(String message);

    @Query(value = "SELECT * FROM product_feature_value pfv WHERE pfv.unit_of_measure = ?1", nativeQuery = true)
    List<ProductFeatureValue> findByUnitOfMeasure(String unitOfMeasure);

    @Query(value = "SELECT pfv.* FROM product_feature_value pfv WHERE pfv.feature_id = ?1", nativeQuery = true)
    List<ProductFeatureValue> findByFeature(ProductFeature feature);

    @Query(value = "SELECT pfv.* FROM product_feature_value pfv WHERE pfv.feature_id = ?1 AND pfv.attribute_values::text LIKE ?2", nativeQuery = true)
    List<ProductFeatureValue> findByFeatureIdAndJsonPattern(UUID featureId, String jsonPattern);

    @Query(value = "SELECT * FROM product_feature_value pfv WHERE pfv.product_id = ?1", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM product_feature_value pfv WHERE pfv.status = ?1", nativeQuery = true)
    ...
    @Query("DELETE FROM ProductFeatureValue pfv WHERE pfv.feature.id = :featureId")
    void deleteByFeatureMappingFeatureId(@Param("featureId") UUID featureId);

    @Query("SELECT pfv FROM ProductFeatureValue pfv WHERE pfv.feature.id = :featureId AND pfv.attributeValues IS NOT NULL")
